...
* :class:`KoyotePythonREPLTool` : a drop-in replacement for ``PythonREPLTool``
  that executes the code snippet inside a kernel-enforced Koyote
  compartment instead of ``exec()`` in the caller's process.
* :class:`KoyoteGraphNode` : wraps any LangGraph execution node so its body
  runs inside an :class:`koyote.AgentKoyote` compartment, with
...
class KoyotePythonREPLTool(BaseTool):  # type: ignore[misc]
    ...
    description: str = (
        ...
        "Runs inside a kernel-enforced Koyote sandbox."
    )
...
class KoyoteGraphNode:
    """Wrap a function so it runs inside a Koyote compartment.

    Turns any callable into a LangGraph node that executes within a
    kernel-enforced :class:`koyote.AgentKoyote`. The wrapped node calls
    ...
    def _permissions_from(self, metadata: Optional[Mapping[str, Any]]) -> tuple[str, ...]:
        ...
        if metadata:
            listed = metadata.get("permissions") or metadata.get("koyote_permissions")
            if listed:
    ...
    def attach(
        ...
        *,
        ...
        if builder is None:
            raise TypeError(
                "KoyoteGraphNode.attach requires langgraph; install it with"
                " 'pip install langgraph'."
            )
        node_name = name or getattr(self._fn, "__name__", "koyote_node")
