...
or LLM-authored code inside a :class:`koyote.AgentKoyote` compartment.
...
:class:`koyote.sandbox.enforcer.SandboxEnforcer` still enforces the
...
_logger = logging.getLogger("koyote.hooks")
...
class ExecutionResult:
    """Structured outcome of running code inside a Koyote compartment.
...
class SandboxRunner:
    """Execute shell commands or code snippets inside an AgentKoyote.

    A fresh :class:`koyote.AgentKoyote` is created per :meth:`run`
    ...
    credential_rules
        ...
        :class:`koyote.sandbox.proxy.RouteConfig`).
    sandbox
    ...
    def _box(self) -> AgentKoyote:
        return AgentKoyote(config=KoyoteConfig(
    ...
    def run_code(
        ...
        *,
        ...
        The snippet is written under ``.koyote/tmp`` (outside the diff
        ...
        tmp_root = os.path.join(self.workdir, ".koyote", "tmp")
