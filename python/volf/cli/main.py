...
from volf.hooks.base import SandboxRunner, diff_trees, index_workdir
...
from volf.engine.execution import Execution, ExecutionManager, ExecutionKind, ExecutionStatus
...
from volf.audit import changed_since_index, run_audit
from volf.drift import detect_changes, detect_drift
from volf.graph import build_dependency_graph
from volf.github.installations import list_ready_repos, store_dir as installations_store_dir
from volf.github.webhook_server import WebhookServer
from volf.intelligence import resolve_migration
from volf.knowledge import ensure_test_recipe
from volf.maintenance import get_migration_history, run_maintenance_cycle
...
from volf.change_source import NO_IMPACT
...
from volf.intelligence import VolfIntelligence
from volf.github.client import GitHubAppClient
from volf.github.pr_bot import make_pr_bot_handler, run_on_pr_locally
from volf.github.provisioning import workdir_for_event
from volf.github.watch import watch_once
from volf.mcp_server import serve_stdio

_logger = logging.getLogger("volf.cli")

VOLF_DIR = ".volf"
TOPOLOGY_FILE = os.path.join(VOLF_DIR, "topology.json")
CONFIG_FILE = os.path.join(VOLF_DIR, "config.yaml")
...
def _load_topology() -> dict:
    if not os.path.exists(TOPOLOGY_FILE):
        print(f"Error: Not a volf project (missing {TOPOLOGY_FILE}). Run 'volf init' first.")
        sys.exit(1)
...
_SHIM_TEMPLATE = """\
#!/usr/bin/env bash
# Volf workspace shim for: {agent}
# Auto-generated by `volf init`. Do NOT edit by hand.
set -e

_VOLF_REAL="{real_path}"
...
if [ -n "$_ws" ]; then
  if [ -n "${{VOLF_EXECUTION_ID:-}}" ]; then
    # Already inside a Volf execution (an agent launched another agent).
    # Inherit the parent execution's kernel boundary - do not wrap again.
    exec "$_VOLF_REAL" "$@"
  fi
  # Delegate to the Volf exec shim, which applies the compartment policy
  # and records the execution as an AgentSession.
  exec python3 -m volf._exec_shim "{agent}" "$_ws" "$@"
else
...
# Volf Workspace Configuration
# See https://github.com/Devaretanmay/Volf for docs.
...
# Declared workflow DAGs - run with `volf workflow run <name>`.
...
def _resolve_real_binary(agent_name: str, workspace_root: str) -> str | None:
    """Resolve the real agent binary, excluding Volf's own shim dir from PATH."""
    shim_dir = os.path.abspath(os.path.join(workspace_root, ".volf", "bin"))
...
def _write_shims(volf_dir: str) -> list[str]:
    """Detect installed agents and write shim scripts into .volf/bin/."""
    bin_dir = os.path.join(volf_dir, "bin")
...
def cmd_init(args):
    """Initialize a Volf workspace in the current directory (legacy advanced path)."""
    print("Note: `volf init` is the legacy workspace path. Normal onboarding is:")
    print("  volf auth → volf index → volf check → volf fix")
    print()
    for sub in ("state", "logs", "snapshots", "sessions", "lanes", "executions", "integration"):
        os.makedirs(os.path.join(VOLF_DIR, sub), exist_ok=True)
    ...
    created_shims = _write_shims(VOLF_DIR)

    project_name = os.path.basename(os.path.abspath("."))
    print(f"Initialized Volf workspace in {os.path.abspath(VOLF_DIR)}/\n")
    ...
    print("Commands:")
    print("  Run an agent:      volf claude | opencode | codex")
    print("  Create a workflow: volf -w <name>")
    print("  Add steps:         volf step <name> <file_or_dir>")
    print("  Run workflow:      volf run <name>")
...
def cmd_status(args):
    ...
    if not ws_root:
        print("Error: Not inside a Volf workspace. Run 'volf init' first.")
        sys.exit(1)
...
def cmd_doctor(args):
    ...
    graph_path = os.path.join(ws_root, ".volf", "graph.json")
    indexed = os.path.isfile(graph_path)
    try:
        tcmd = _detect_test_command(ws_root)
    except Exception:
        tcmd = ""
    gh_connected = bool(os.environ.get("GITHUB_TOKEN") or os.environ.get("VOLF_GITHUB_TOKEN")
                        or (os.environ.get("VOLF_GITHUB_APP_ID") and os.environ.get("VOLF_GITHUB_PRIVATE_KEY")))
    kb_state = "MISSING"
    if indexed:
        try:
    ...
    try:
        ...
        pass
    print("================================================================================")
    print("                 VOLF DOCTOR — product readiness                              ")
    print("================================================================================\n")
    ...
    if summary.get("configured"):
        ...
        print("AI provider:        NOT CONFIGURED — run `volf auth` (needed only for AI repair)")
    print(f"Repository:         {ws_root}")
    print(f"Indexed:            {'YES' if indexed else 'NOT INDEXED — run `volf index`'}")
...
def _snapshot_worktree(workspace_root: str, snapshot_id: str) -> str:
    """Copy the current worktree into ``.volf/snapshots/<snapshot_id>/``.
    ...
    """
    snap_dir = os.path.join(workspace_root, ".volf", "snapshots", snapshot_id)
...
def _launch_agent(
    ...
    missing).
    """
    cfg = load_config(os.path.join(workspace_root, ".volf", "config.yaml"))
    ...
    if agent_name == "opencode":
        xdg_base = os.path.join(workspace_root, ".volf", "xdg")
    ...
    extra_env["VOLF_EXECUTION_ID"] = execution.execution_id
...
def cmd_exec_shim(args):
    """Internal: called by workspace shim scripts to launch an agent under Volf.

    Usage: volf _exec_shim <agent_name> <workspace_root> [argv...]
    """
...
def cmd_inspect(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg_path = os.path.join(ws_root, ".volf", "config.yaml")
    ...
    if getattr(args, "json", False):
        ...
        print(f"Volf Workspace: {project_name}\n")
        print("COMPARTMENTS\n")
...
def cmd_run(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg = load_config(os.path.join(ws_root, ".volf", "config.yaml"))
    if cfg.workflows:
        print("Available Workflows:")
        for wname in sorted(cfg.workflows):
            print(f"  - {wname:<24} (run with: volf run {wname})")
        return
    ...
    if os.path.exists(TOPOLOGY_FILE):
        ...
        if comps:
            ...
            print("Use `volf workflow show` or `volf check` for maintenance. `volf run <workflow>` to execute.")
            print("No fallback execution performed.")
            return

    print("No workflow specified. Usage: volf run <workflow_name>")


def cmd_exec(args):
    """Run a command or launch an agent inside an explicitly selected compartment."""
    if not args.cmd:
        print("Error: No command provided to exec. Usage: volf exec [--compartment X] <command>")
        sys.exit(1)
    ...
    result = runner.run(
        ...
        env={"VOLF_EXECUTION_ID": execution.execution_id},
    )
    if result.stdout:
        print(result.stdout)
    if result.stderr:
...
def cmd_wrap(args):
    ...
    if args.claude is not None:
        ...
        xdg_base = os.path.join(ws_root, ".volf", "xdg")
        for sub in ["config", "data", "cache", "state"]:
            os.makedirs(os.path.join(xdg_base, sub), exist_ok=True)
        env_vars.update({
            "XDG_CONFIG_HOME": os.path.join(xdg_base, "config"),
    ...
    env_vars["VOLF_EXECUTION_ID"] = execution.execution_id

    print(f"Volf Agent Session #{session.session_id} active on Virtual Agent Lane '{lane_id}'.")
...
def cmd_lanes(args):
    ...
    lanes = lane_mgr.list_lanes()
    if not lanes:
        print("No virtual agent lanes found. Run 'volf lane create <name>' or 'volf wrap --lane <name>' first.")
        return
...
def cmd_lane_inspect(args):
    ...
    if args.json:
        ...
        print("================================================================")
        print(f"              VOLF VIRTUAL AGENT LANE #{lane.lane_id}        ")
        print("================================================================")
...
def cmd_sessions(args):
    ...
    if not sessions:
        print("No recorded agent sessions found. Run 'volf wrap' or 'volf run' first.")
        return
...
def _run_declared_workflow(ws_root: str, wf: WorkflowConfig, cfg: WorkspaceConfig) -> None:
    ...
    exec_mgr = ExecutionManager(workdir=ws_root)
    print(f"\nVOLF WORKFLOW: {wf.name}  ({len(nodes)} node(s))\n")
    for n in nodes:
    ...
    for n in nodes:
        ...
        result = runner.run(
            ...
            env={"VOLF_EXECUTION_ID": ex.execution_id},
        )
        if result.stdout:
...
def _run_workflow_file(ws_root: str, workflow_file: str, compartment_id: str, verbose: bool) -> None:
    ...
    print(f"Volf Workflow Execution #{execution.execution_id}")
...
def cmd_workflow_branch(args):
    ...
    if not name:
        print("Error: Workflow branch name required. Usage: volf workflow branch <name>")
        sys.exit(1)
    ...
    print(f"  To add steps:  volf step {name} <file_or_command>")
    print(f"  To execute:    volf workflow run {name}")
...
def cmd_diff(args):
    ...
    print("\nVOLF DIFF - changes associated with agent executions\n")
    if not records:
        print("  (no change sets recorded - run an agent under Volf first)")
        print()
        return
...
def _git_commit_execution(
    ...
    msg_header = user_message or f"Volf: apply changes from {ex.agent_name} ({ex.execution_id})"
...
def cmd_apply(args):
    ...
    if getattr(args, "commit", False):
        print("\nCreating Git commit(s) with Volf metadata trailers...")
...
def cmd_undo(args):
    """Reverse the last Volf-managed apply operation and restore the worktree."""
    ws_root = find_workspace_root()
    if not ws_root:
        print("Error: Not inside a Volf workspace. Run 'volf init' first.")
        sys.exit(1)
...
def cmd_inventory(args):
    ...
    if critical > 0:
        ...
        print("   Run `volf autopatch` to generate verified migration PRs.")
...
def cmd_verify(args):
    ...
    print("================================================================================")
    print("                    VOLF AUTOPATCH: BEHAVIORAL VERIFIER                      ")
    print("================================================================================\n")
...
def _print_auth_warning():
    print("================================================================================")
    print("                VOLF: AI PROVIDER AUTHENTICATION REQUIRED                   ")
    print("================================================================================\n")
    print("[ERROR] No AI provider configured.\n")
    print("Volf requires an AI provider to analyze dependencies and maintain your code.")
...
def cmd_reviews(args):
    """List past Volf maintenance runs from the repository ledger."""
    root_path = os.path.abspath(getattr(args, "path", ".") or ".")
    history = get_migration_history(root_path)
    if not history:
        print("No Volf maintenance runs recorded for this repository yet.")
        print("Run `volf fix` to create the first entry.")
        return
    print(f"Volf maintenance runs ({len(history)}):")
...
def cmd_auth(args):
    ...
    if getattr(args, "status", False):
        summary = get_active_provider_summary()
        print("================================================================================")
        print("                 VOLF: AI PROVIDER CREDENTIAL STATUS                         ")
        print("================================================================================\n")
        if summary["configured"]:
            ...
            print("Action:       Run 'volf auth' to connect an AI provider.")
        print("\n================================================================================")
        return

    if getattr(args, "clear", False):
        clear_credentials()
        print("[OK] Stored Volf credentials removed.")
        return
    ...
    if not provider or not api_key:
        print("================================================================================")
        print("                   VOLF: CONNECT YOUR AI PROVIDER                            ")
        print("================================================================================\n")
        ...
        if not sys.stdin.isatty() and (not provider or not api_key):
            ...
            print("  volf auth --provider <anthropic|openai> --api-key <sk-...>")
            sys.exit(1)
    ...
    root_path = os.path.abspath(getattr(args, "path", ".") or ".")
    print(f"\n[INDEXING] Initializing Volf Knowledge Graph for: {root_path}...")
    try:
        ...
        print("\nNext steps:")
        print("  1. Run `volf check` to inspect external dependencies and drift.")
        print("  2. Run `volf fix` to autonomously resolve migrations.")
...
def cmd_consult(args):
    ...
    if not has_valid_credentials():
        _print_auth_warning()
        print("Consult reasons with AI — static `volf check` needs no key.")
        sys.exit(1)

    detections = [d for d in detect_changes(root_path) if d.outcome != NO_IMPACT]
    if not detections:
        print("Volf Consult: no maintenance problems found. No changes made.")
        return
    ...
    title = f"[Volf Consult] {len(items)} maintenance issue(s) in {repo}"
    resp = client.create_issue(repo=repo, title=title, body=body, labels=["volf", "consult"])
...
def cmd_app(args):
    ...
    if action == "serve":
        ...
        secret = args.secret or os.environ.get("VOLF_WEBHOOK_SECRET")
        if not secret and not getattr(args, "no_secret", False):
            ...
            print("Set VOLF_WEBHOOK_SECRET, pass --secret, or use --no-secret for local debugging only.")
            sys.exit(2)
        ...
        print("================================================================================")
        print("               VOLF GITHUB APP: CONTINUOUS WEBHOOK LISTENER                  ")
        print("================================================================================\n")
        ...
        print("[Volf App] Status: Ready (Daemon not running locally).")


def cmd_mcp(args):
    """Run Volf Model Context Protocol (MCP) server over stdio."""
    serve_stdio()
...
def cmd_maintain(args):
    ...
    print("================================================================================")
    print("               VOLF AUTONOMOUS MAINTENANCE LOOP: EXECUTION                   ")
    print("================================================================================\n")
    ...
    if not target_provider or target_provider == "auto":
        ...
        if detected_all:
            ...
            print("Run `volf check` to audit, or pass --provider explicitly.")
            print("================================================================================")
            return
    ...
    if getattr(report, "error", None):
        ...
        print("Action required:       Run `volf auth` to enable AI repair.")
...
def cmd_graph(args):
    ...
    print("================================================================================")
    print("                 VOLF: EXTERNAL-CHANGE DEPENDENCY GRAPH                     ")
    print("================================================================================\n")
...
def cmd_providers(args):
    ...
    print("================================================================================")
    print("                    VOLF PROVIDER CONTRACT REGISTRY                          ")
    print("================================================================================\n")
    for p in providers:
...
def cmd_pr(args):
    """Review a pull request using the Volf PR Bot."""
    ...
    print("================================================================================")
    print("                      VOLF AUTOMATED PR CONTRACT GUARD                       ")
    print("================================================================================\n")
...
def main():
    # Direct agent shortcut: `volf claude` / `volf opencode` / `volf codex`
    ...
    description = textwrap.dedent("""\
        Volf: Autonomous External-Change Intelligence & Controlled Execution
        ...
        Core Commands (maintenance product — keeps software working when systems around it change):
          ...
          volf check [path]             Detect contract changes & impact (read-only, alias: scan, audit)
          ...
          volf app serve                Run GitHub App webhook listener
          volf pr                       Review a pull request with contract guard

        Legacy / advanced (workflows, sessions, lanes):
          volf init | status | diff | apply | commit | undo | graph | exec | mcp
    """)
    ...
    parser.add_argument(
        ...
        dest="workflow_flag",
        default=None,
        help="Create a new workflow branch (e.g. volf -w invoice-pipeline)",
    )
    parser.add_argument(
        "--run",
        dest="run_flag",
        default=None,
        help="Run a workflow DAG directly (e.g. volf --run invoice-pipeline)",
    )
    subparsers = parser.add_subparsers(dest="command", help=argparse.SUPPRESS)

    subparsers.add_parser("init", help="Initialize a Volf workspace in the current directory")
    ...
    apply_parser.add_argument("-c", "--commit", action="store_true", help="Automatically create a Git commit with Volf trailers")
    ...
    undo_parser = subparsers.add_parser("undo", help="Reverse the last Volf-managed apply operation")
    ...
    explain_p = subparsers.add_parser("explain", help="Explain why Volf classified a finding as affected, unaffected, or unresolved")
    ...
    check_p.add_argument("--write-graph", action="store_true", help="Persist .volf/graph.json")
    ...
    scan_p.add_argument("--write-graph", action="store_true", help="Persist .volf/graph.json")
    ...
    audit_p.add_argument("--write-graph", action="store_true", help="Persist .volf/graph.json")
    ...
    reviews_p = subparsers.add_parser("reviews", help="List past Volf maintenance runs")
    ...
    app_p = subparsers.add_parser("app", help="Manage Volf GitHub App and Webhook server")
    ...
    subparsers.add_parser("mcp", help="Run Volf Model Context Protocol (MCP) server for AI assistants")
    ...
    index_p.add_argument("--write-graph", action="store_true", default=True, help="Persist .volf/graph.json")
