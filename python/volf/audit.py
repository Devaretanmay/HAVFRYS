...
from volf.graph import audit_dependency_graph, build_dependency_graph


def render_audit_cli(summary: Dict[str, Any]) -> str:
    lines = []
    lines.append("=" * 80)
    lines.append("         VOLF: EXTERNAL-CHANGE DEPENDENCY AUDIT & RISK REGISTER")
    lines.append("=" * 80)
    ...
    lines.append("=" * 80)
    lines.append("Run `volf fix <path> --provider <name>` to execute autonomous migration.")
    lines.append("=" * 80)
    return "\n".join(lines)
...
def render_audit_github_issue(summary: Dict[str, Any]) -> str:
    lines = []
    lines.append("# Volf: External Dependency Map & Risk Register\n")
    lines.append(f"Volf mapped **{summary.get('total_callsites_mapped', 0)} external API touchpoints** across **{summary.get('total_providers_detected', 0)} providers** in this repository.\n")
    ...
    if at_risk:
        ...
        lines.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
        for item in at_risk:
            lines.append(f"| **{item['provider_name']}** | `{item['package_name']}` | `{item['current_version']}` -> `{item['target_version']}` | **{item['callsites_count']} callsites** ({len(item['affected_files'])} files) | {item['breaking_change']} | `volf fix --provider {item['provider_name'].lower()}` [MERGE_READY] |")
        lines.append("")
    ...
    lines.append("---")
    lines.append("> *Generated automatically by Volf External-Change Intelligence.*")
    return "\n".join(lines)
...
def _changed_since(repo_root: str, state: Dict[str, Any]) -> List[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root, topdown=True):
        dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", ".next", "__pycache__", ".venv", "target", ".volf"}]
        for fn in filenames:
...
def read_index_state(repo_root: str = ".") -> Optional[Dict[str, Any]]:
    p = os.path.join(repo_root, ".volf", STATE_FILE)
...
def write_index_state(repo_root: str, summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    os.makedirs(os.path.join(repo_root, ".volf"), exist_ok=True)
    with open(os.path.join(repo_root, ".volf", STATE_FILE), "w", encoding="utf-8") as f:
...
def run_audit(repo_root: str = ".", output_format: str = "cli", write_graph: bool = False) -> str:
    ...
    if write_graph:
        ...
        os.makedirs(os.path.join(repo_root, ".volf"), exist_ok=True)
        with open(os.path.join(repo_root, ".volf", "graph.json"), "w") as f:
