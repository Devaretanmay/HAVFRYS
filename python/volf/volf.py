"""Volf - kernel-level sandbox with an insulated runtime for isolated execution.

Two entry points:

* :class:`Volf` : a normal outer compartment. A kernel sandbox plus a runtime that
...
* :class:`AgentVolf` : an agent outer compartment (agent-oriented container). It auto-loads every
...
_logger = logging.getLogger("volf")
...
class Volf:
    ...
    def __init__(
        self,
        workdir: str = ".",
        config: Optional[VolfConfig] = None,
        verbose: bool = False,
    ):
        if config is not None:
            self.config = config
        else:
            self.config = VolfConfig(workdir=workdir)
        self.workdir = os.path.abspath(self.config.workdir)
        self._snapshot_base = os.path.join(self.workdir, self.VOLF_DIR, "snapshots")
    ...
    def add(self, compartment: Compartment) -> "Volf":
    ...
    def edge(self, from_name: str, to_name: str) -> "Volf":
    ...
    def register_module(self, module_cls) -> "Volf":
    ...
    def run(
        ...
        VolfResult
        """
        task_desc = request or entry or "volf.run()"
        self._started_at = time.time()
        _logger.info("Volf %s running: %s", self.box_id, task_desc[:80])
    ...
    def _build_result(self, raw: dict[str, Any], elapsed: float) -> VolfResult:
        errors: list[str] = []
        completed: list[str] = []
        status = "success"
...
class AgentVolf(Volf):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            config = VolfConfig(workdir=workdir, auto_modules=True)
