"""Volf CI Step Runner & Wrapper.

Allows 1-word or 1-line execution of CI pipeline steps inside a kernel-enforced
Volf compartment. Preserves stdout/stderr and exit codes for CI runners.
"""
...
from volf.volf import AgentVolf, VolfConfig
...
class VolfCIRunner:
    ...
    def run_step(self, cmd: str, name: str = "ci_step") -> dict[str, Any]:
        """Runs a command inside a kernel-enforced Volf CI compartment."""
...
def run_ci_step(cmd: str, block_network: bool = True, sandbox: bool = True, timeout_s: int = 600) -> int:
    ...
    runner = VolfCIRunner(block_network=block_network, sandbox=sandbox, timeout_s=timeout_s)
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="volf",
        description="Volf CI Runner - 1-word drop-in kernel sandbox for CI pipeline steps.",
    )
