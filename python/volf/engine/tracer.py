"""Execution trace - makes the Volf lifecycle visible for development."""
...
def is_trace_enabled() -> bool:
    return os.environ.get("VOLF_TRACE", "0") in ("1", "true", "yes", "on")
...
class Tracer:
    ...
    def header(self, request: str) -> None:
        ...
        lines = [
            ...
            f"  {_colorize(f'Volf #{cid}', 'bold')}    {_colorize(ts, 'dim')}",
            f"  {_colorize(sep, 'cyan')}",
        ]
    ...
    def _format_event(self, event: str, elapsed: float, data: dict) -> Optional[str]:
        ...
        if event == "box.created":
            path = data.get("path", "")
            return f"{prefix}  {_colorize('[ok]', 'green')} Volf Created     {_colorize(path, 'dim')}"
        ...
        if event == "box.destroyed":
            return f"{prefix}  {_colorize('[ok]', 'green')} Volf Released"
