...
try:
    from volf._core import sandbox_apply as _core_sandbox_apply
except ImportError:
    _core_sandbox_apply = None

_logger = logging.getLogger("volf.process_runner")
...
class PtySupervisor:
    ...
    def _resolve(self, binary_name: str) -> str:
        ...
        volf_base = os.path.abspath(os.path.join(self.workdir, ".volf"))
        path_dirs = os.environ.get("PATH", "").split(os.path.pathsep)
        for d in path_dirs:
            if not d:
                continue
            abs_d = os.path.abspath(d)
            if abs_d == volf_base or abs_d.startswith(volf_base + os.sep):
                continue
