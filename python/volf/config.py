"""Volf workspace configuration loader.

Reads `.volf/config.yaml` and returns typed config objects for compartments,
...
@dataclass
class BotConfig:
    """Volf GitHub App behavior policy.
...
def load_config(config_path: Optional[str] = None) -> WorkspaceConfig:
    """Load `.volf/config.yaml`.  Returns safe defaults when not found."""
    if config_path is None:
        config_path = os.path.join(".volf", "config.yaml")
    ...
    if not _YAML_AVAILABLE:
        warnings.warn(
            ...
            "to use .volf/config.yaml. Falling back to default policy.",
            RuntimeWarning,
            stacklevel=2,
        )
        return _default_config()
    ...
    for dir_name in ("workflows", os.path.join(".volf", "workflows")):
...
def is_volf_workspace(path: Optional[str] = None) -> bool:
    """Return True if *path* (or cwd) is inside a Volf workspace."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".volf")):
            return True
...
def find_workspace_root(path: Optional[str] = None) -> Optional[str]:
    """Walk up the directory tree looking for a .volf/ directory."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".volf")):
            return check
