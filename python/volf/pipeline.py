...
"""
Volf event model and shared maintenance pipeline.
...
from volf.github.client import GitHubAppClient
from volf.credentials import has_valid_credentials
...
from volf.autopatch import (
    ScanConfig,
    scan_callsites,
)
from volf.providers.registry import get_default_registry, ProviderSpec
from volf.drift import detect_drift
from volf.formatters import run_style_formatter
from volf.test_runner import (
    _detect_test_command,
    _run_tests,
    _compute_lockfile_hash,
    _blake3_digest,
)
from volf.intelligence import VolfIntelligence, resolve_migration
from volf.knowledge import direct_rewrites_for, upsert_learned as kb_upsert
from volf.ai_planner import AIPatchPlanner, ai_followup_for_missed, build_reasoning_context
from volf.maintenance_agents import ImpactAnalyst
from volf.patch_writer import apply_rewrites, discover_aliases, instantiate_alias_rules
from volf.sandbox.snapshot import SnapshotManager

_logger = logging.getLogger("volf.pipeline")
    ...
    def run(self, ctx: TriggerContext) -> "PipelineResult":
        ...
        if not analysis.has_findings and not self.policy.always_report_clean:
            return PipelineResult(
                ...
                status="clean",
                comment_body="",
                status_description="Volf: no external contract impact detected",
            )
    ...
    def _run_consult(self, ctx: TriggerContext, analysis: AnalysisResult) -> "PipelineResult":
        ...
        if not has_valid_credentials():
            return PipelineResult(
                ...
                analysis=analysis,
                status="refused",
                comment_body="Volf Consult requires AI reasoning — no provider configured. "
                             "Run `volf auth` to connect one.",
                status_description="Volf Consult refused: no AI provider configured",
            )
        ...
        return PipelineResult(
            ...
            status="consulted",
            comment_body=body,
            status_description=f"Volf Consult: assessed {len(items)} maintenance issue(s), no code modified",
        )
...
def apply_fixes(
    ...
    """Apply fixes via Volf Intelligence: DIRECT (registry+KB) or AI fallback. Single router."""
    modified_files: List[str] = []
    unified_diffs: List[str] = []

    snapshot_dir = os.path.join(ctx.workdir, ".volf", "snapshot_tmp")
...
def surface_result(
    ...
    if analysis.verified and analysis.modified_files:
        ...
        if analysis.test_command and analysis.test_exit_code == 0:
            status_desc = f"Volf: autonomous repair verified ({len(analysis.modified_files)} file(s) patched, tests green)"
        else:
            status_desc = f"Volf: autonomous repair applied ({len(analysis.modified_files)} file(s) patched, test suite unconfigured)"
        ...
        status_desc = f"Volf found {len(analysis.findings)} maintenance issue(s)"
    else:
        comment = render_verification_comment(analysis, ctx)
        status_desc = "Volf: no external contract impact detected"
