...
from volf.drift import detect_drift  # noqa: F401
...
from volf.git_ops import git_commit_and_push, gh_create_pr
from volf.maintenance_agents import ImpactAnalyst
from volf.patch_writer import (
    apply_rewrites, discover_aliases, instantiate_alias_rules, PatchResult,
)
from volf.providers.registry import get_default_registry
...
from volf.knowledge import direct_rewrites_for, record_failure, upsert_learned as kb_upsert
...
def record_migration_history(repo_dir: str, record: Dict[str, Any]) -> None:
    ...
    history_dir = os.path.join(repo_dir, ".volf")
...
def get_migration_history(repo_dir: str) -> List[Dict[str, Any]]:
    """Retrieve verified migration history records from .volf/history.json."""
    history_file = os.path.join(repo_dir, ".volf", "history.json")
    if os.path.exists(history_file):
        try:
            with open(history_file, "r") as f:
                return json.load(f)
...
def run_maintenance_cycle(
    ...
    **_ignored: Any,
    ...
    snapshot_dir = os.path.join(repo_dir, ".volf", "snapshot_tmp")
    ...
    if decision.strategy == "DIRECT":
        ...
        if ai_planner is None:
            quarantine_error = (
                ...
                "Run `volf auth` or set ANTHROPIC_API_KEY / OPENAI_API_KEY."
            )
        else:
        ...
        quarantine_error = (
            ...
            "Run `volf auth` to enable AI repair, or add a verified migration to the registry."
        )
    ...
    for dirpath, dirnames, filenames in os.walk(repo_dir, topdown=True):
        dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", ".next", "__pycache__", ".volf"}]
        for fn in filenames:
    ...
    if create_pr and github_repo and files_modified > 0:
        branch_name = f"volf/{p_spec.name}-v{actual_to.replace('.', '-')}"
        commit_msg = (
            f"migrate: {p_spec.display_name} {actual_from} -> {actual_to}\n\n"
            f"Detected and patched by Volf autonomous maintenance engine.\n"
            f"Rules applied:\n" + "\n".join(f"- {d}" for d in all_rules)
        )
        pushed = git_commit_and_push(repo_dir, modified_paths, branch_name, commit_msg)

        if pushed:
            pr_title = f"volf: migrate {p_spec.display_name} {actual_from} -> {actual_to}"
        ...
        if not pr_url and github_client:
            pr_resp = github_client.create_pull_request(
                ...
                labels=["volf-maintenance", "verified-green"],
            )
