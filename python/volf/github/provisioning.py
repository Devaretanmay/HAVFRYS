...
otherwise (public repos), or VOLF_REPO_REMOTE_* overrides (tests,
mirrors, self-hosted).
"""
...
_logger = logging.getLogger("volf.provisioning")


def cache_dir() -> str:
    base = os.environ.get("VOLF_REPOS_DIR",
                           os.path.join(os.path.expanduser("~/.volf"), "repos"))
...
def remote_for(repo_full_name: str, token: Optional[str] = None) -> str:
    ...
    override = os.environ.get(f"VOLF_REPO_REMOTE_{slug}")
    if override:
        return override
    if token:
