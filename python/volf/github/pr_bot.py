...
"""
Volf GitHub App PR bot.

This is the first product milestone: install the app, open a PR, and Volf
...
from volf.ai_planner import AIPatchPlanner, build_reasoning_context
...
from volf.github.installations import (
    REPO_INDEXED, REPO_PENDING, REPO_READY, record_installation_event, set_repo_state,
)
from volf.github.provisioning import resolve_pr_workdir
from volf.graph import build_dependency_graph
from volf.drift import detect_drift

_logger = logging.getLogger("volf.pr_bot")
...
def run_on_pr_locally(
    ...
    Useful for:
    * local development
    * CI check jobs that want Volf to post results
    """
    ...
    payload: Dict[str, Any] = {
        ...
        "volf": {"changed_files": changed_files},
    }
...
def render_day0_onboarding_issue(repo: str, workdir: Optional[str] = None) -> str:
    ...
    lines = [
        "-----------------------------------------",
        "   VOLF DAY-0 REPOSITORY ONBOARDING   ",
        "-----------------------------------------",
        "",
        f"Volf has mapped external API dependencies and contracts for `{repo}`.",
        "",
        ...
        "  - [OK] Auto-Fix Policy: Opt-in (Configurable via .volf/config.yaml)",
...
def handle_installation_event(
    ...
    for repo_info in repos_data:
        ...
        try:
            client.create_issue(
                repo=repo_name,
                title="[VOLF] Day-0 External Contract & API Dependency Map",
                body=issue_body,
                labels=["volf", "maintenance"],
            )
...
def handle_issue_comment_event(
    ...
    """Handle issue_comment.* events mentioning @volf.

    `@volf` alone re-runs the pipeline on the PR; `@volf explain`
    ...
    if not repo or not number or not body or "@volf" not in body.lower():
    ...
    if "@volf explain" in body.lower():
        ...
        reply = render_consult_issue(items) if items else (
            "Volf checked this PR's external API touchpoints. "
