...
def render_verification_comment(
    ...
    return f"Volf checked {touchpoints} external API touchpoint(s). No contract violations detected. No changes made."
...
def render_maintenance_issue_comment(
    ...
    lines: List[str] = [
        "-----------------------------------------",
        "        VOLF FOUND A MAINTENANCE ISSUE",
        "-----------------------------------------",
        "",
    ]
...
def _render_single_finding(
    ...
    lines.append("")
    lines.append("**Volf can repair this automatically.**")
    lines.append("")
    return lines
...
def render_pr_summary(analysis: "AnalysisResult", ctx: "TriggerContext") -> str:
    ...
    if not analysis.has_findings:
        touchpoints = analysis.callsites_total or len(analysis.providers_detected) or 1
        return (f"## Volf review: no contract impact\n\n"
    ...
    parts = [f"## Volf review: {len(analysis.findings)} maintenance issue(s)", ""]
    for finding in analysis.findings:
...
def render_pr_footer(ctx: "TriggerContext") -> str:
    ...
    lines.append("Comment `@volf` to re-run this review.")
    return "\n".join(lines)
...
def render_consult_issue(items: List[dict]) -> str:
    ...
    lines: List[str] = [
        "-----------------------------------------",
        "   VOLF CONSULT: MAINTENANCE ADVISORY ",
        "-----------------------------------------",
        "",
    ]
    for item in items:
        ...
        if item.get("auto_repairable"):
            lines.append("Volf can repair this automatically (`volf fix`).")
        else:
            lines.append("Volf cannot repair this automatically — manual review advised.")
