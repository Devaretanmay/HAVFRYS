"""GitHub Webhook Daemon & Event Ingestion Server for Volf."""
...
class WebhookHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health" or self.path == "/":
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"status": "healthy", "service": "volf-github-app"}).encode("utf-8"))
        else:
...
class WebhookServer:
    ...
    def __init__(
        ...
        self.port = port
        self.host = host
        self.secret = secret or os.environ.get("VOLF_WEBHOOK_SECRET")
    ...
    def start(self, blocking: bool = False):
        ...
        if blocking:
            print(f"[Volf GitHub App] Listening for webhooks on http://{self.host}:{self.port}/webhook")
