...
or LLM-authored code inside a :class:`volf.AgentVolf` compartment.
...
:class:`volf.sandbox.enforcer.SandboxEnforcer` still enforces the
...
from ..volf import AgentVolf, VolfConfig
...
_logger = logging.getLogger("volf.hooks")
...
class ExecutionResult:
    """Structured outcome of running code inside a Volf compartment.
...
class SandboxRunner:
    """Execute shell commands or code snippets inside an AgentVolf.

    A fresh :class:`volf.AgentVolf` is created per :meth:`run`
    ...
    credential_rules
        ...
        :class:`volf.sandbox.proxy.RouteConfig`).
    sandbox
    ...
    def _box(self) -> AgentVolf:
        return AgentVolf(config=VolfConfig(
    ...
    def run_code(
        ...
        *,
        ...
        The snippet is written under ``.volf/tmp`` (outside the diff
        ...
        tmp_root = os.path.join(self.workdir, ".volf", "tmp")
