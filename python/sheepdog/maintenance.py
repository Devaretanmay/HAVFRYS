...
from sheepdog.patch_writer import (
    apply_rewrites, discover_aliases, instantiate_alias_rules, PatchResult,
)
from sheepdog.providers.registry import get_default_registry
from sheepdog.sandbox.snapshot import SnapshotManager, _file_hash

from sheepdog.intelligence import SheepdogIntelligence, resolve_migration
from sheepdog.knowledge import direct_rewrites_for, record_failure, upsert_learned as kb_upsert
...
def record_migration_history(repo_dir: str, record: Dict[str, Any]) -> None:
    ...
    history_dir = os.path.join(repo_dir, ".sheepdog")
...
def get_migration_history(repo_dir: str) -> List[Dict[str, Any]]:
    """Retrieve verified migration history records from .sheepdog/history.json."""
    history_file = os.path.join(repo_dir, ".sheepdog", "history.json")
    if os.path.exists(history_file):
        try:
            with open(history_file, "r") as f:
                return json.load(f)
...
def run_maintenance_cycle(
    ...
    **_ignored: Any,
    ...
    snapshot_dir = os.path.join(repo_dir, ".sheepdog", "snapshot_tmp")
    ...
    if decision.strategy == "DIRECT":
        ...
        if ai_planner is None:
            quarantine_error = (
                ...
                "Run `sheepdog auth` or set ANTHROPIC_API_KEY / OPENAI_API_KEY."
            )
        else:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_dir, topdown=True):
        dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", ".next", "__pycache__", ".sheepdog"}]
        for fn in filenames:
    ...
    if create_pr and github_repo and files_modified > 0:
        branch_name = f"sheepdog/{p_spec.name}-v{actual_to.replace('.', '-')}"
        commit_msg = (
            f"migrate: {p_spec.display_name} {actual_from} -> {actual_to}\n\n"
            f"Detected and patched by Sheepdog autonomous maintenance engine.\n"
            f"Rules applied:\n" + "\n".join(f"- {d}" for d in all_rules)
        )
        pushed = git_commit_and_push(repo_dir, modified_paths, branch_name, commit_msg)

        if pushed:
            pr_title = f"sheepdog: migrate {p_spec.display_name} {actual_from} -> {actual_to}"
        ...
        if not pr_url and github_client:
            pr_resp = github_client.create_pull_request(
                ...
                labels=["sheepdog-maintenance", "verified-green"],
            )
