...
from sheepdog.hooks.base import SandboxRunner, diff_trees, index_workdir
...
from sheepdog.engine.execution import Execution, ExecutionManager, ExecutionKind, ExecutionStatus
...
from sheepdog.audit import changed_since_index, run_audit
from sheepdog.drift import detect_changes, detect_drift
from sheepdog.graph import build_dependency_graph
from sheepdog.github.installations import list_ready_repos, store_dir as installations_store_dir
from sheepdog.github.webhook_server import WebhookServer
from sheepdog.intelligence import resolve_migration
from sheepdog.knowledge import ensure_test_recipe
from sheepdog.maintenance import get_migration_history, run_maintenance_cycle
...
from sheepdog.change_source import NO_IMPACT
...
from sheepdog.intelligence import SheepdogIntelligence
from sheepdog.github.client import GitHubAppClient
from sheepdog.github.pr_bot import make_pr_bot_handler, run_on_pr_locally
from sheepdog.github.provisioning import workdir_for_event
from sheepdog.github.watch import watch_once
from sheepdog.mcp_server import serve_stdio

_logger = logging.getLogger("sheepdog.cli")

SHEEPDOG_DIR = ".sheepdog"
TOPOLOGY_FILE = os.path.join(SHEEPDOG_DIR, "topology.json")
CONFIG_FILE = os.path.join(SHEEPDOG_DIR, "config.yaml")
...
def _load_topology() -> dict:
    if not os.path.exists(TOPOLOGY_FILE):
        print(f"Error: Not a sheepdog project (missing {TOPOLOGY_FILE}). Run 'sheepdog init' first.")
        sys.exit(1)
...
_SHIM_TEMPLATE = """\
#!/usr/bin/env bash
# Sheepdog workspace shim for: {agent}
# Auto-generated by `sheepdog init`. Do NOT edit by hand.
set -e

_SHEEPDOG_REAL="{real_path}"

_find_workspace() {{
  local dir="$PWD"
  while [ "$dir" != "/" ]; do
    if [ -d "$dir/.sheepdog" ]; then
      echo "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
if [ -n "$_ws" ]; then
  if [ -n "${{SHEEPDOG_EXECUTION_ID:-}}" ]; then
    # Already inside a Sheepdog execution (an agent launched another agent).
    # Inherit the parent execution's kernel boundary - do not wrap again.
    exec "$_SHEEPDOG_REAL" "$@"
  fi
  # Delegate to the Sheepdog exec shim, which applies the compartment policy
  # and records the execution as an AgentSession.
  exec python3 -m sheepdog._exec_shim "{agent}" "$_ws" "$@"
else
...
# Sheepdog Workspace Configuration
# See https://github.com/Devaretanmay/Sheepdog for docs.
...
# Declared workflow DAGs - run with `sheepdog workflow run <name>`.
...
def _resolve_real_binary(agent_name: str, workspace_root: str) -> str | None:
    """Resolve the real agent binary, excluding Sheepdog's own shim dir from PATH."""
    shim_dir = os.path.abspath(os.path.join(workspace_root, ".sheepdog", "bin"))
...
def _write_shims(sheepdog_dir: str) -> list[str]:
    """Detect installed agents and write shim scripts into .sheepdog/bin/."""
    bin_dir = os.path.join(sheepdog_dir, "bin")
...
def cmd_init(args):
    """Initialize a Sheepdog workspace in the current directory (legacy advanced path)."""
    print("Note: `sheepdog init` is the legacy workspace path. Normal onboarding is:")
    print("  sheepdog auth → sheepdog index → sheepdog check → sheepdog fix")
    print()
    for sub in ("state", "logs", "snapshots", "sessions", "lanes", "executions", "integration"):
        os.makedirs(os.path.join(SHEEPDOG_DIR, sub), exist_ok=True)
    ...
    created_shims = _write_shims(SHEEPDOG_DIR)

    project_name = os.path.basename(os.path.abspath("."))
    print(f"Initialized Sheepdog workspace in {os.path.abspath(SHEEPDOG_DIR)}/\n")
    ...
    print("Commands:")
    print("  Run an agent:      sheepdog claude | opencode | codex")
    print("  Create a workflow: sheepdog -w <name>")
    print("  Add steps:         sheepdog step <name> <file_or_dir>")
    print("  Run workflow:      sheepdog run <name>")
...
def cmd_status(args):
    ...
    if not ws_root:
        print("Error: Not inside a Sheepdog workspace. Run 'sheepdog init' first.")
        sys.exit(1)
    ...
    print(f"\nSHEEPDOG WORKSPACE: {project_name}\n")
...
def cmd_doctor(args):
    ...
    graph_path = os.path.join(ws_root, ".sheepdog", "graph.json")
    indexed = os.path.isfile(graph_path)
    try:
        tcmd = _detect_test_command(ws_root)
    except Exception:
        tcmd = ""
    gh_connected = bool(os.environ.get("GITHUB_TOKEN") or os.environ.get("SHEEPDOG_GITHUB_TOKEN")
                        or (os.environ.get("SHEEPDOG_GITHUB_APP_ID") and os.environ.get("SHEEPDOG_GITHUB_PRIVATE_KEY")))
    kb_state = "MISSING"
    if indexed:
        try:
    ...
    try:
        ...
        pass
    print("================================================================================")
    print("                 SHEEPDOG DOCTOR — product readiness                              ")
    print("================================================================================\n")
    ...
    if summary.get("configured"):
        ...
        print("AI provider:        NOT CONFIGURED — run `sheepdog auth` (needed only for AI repair)")
    print(f"Repository:         {ws_root}")
    print(f"Indexed:            {'YES' if indexed else 'NOT INDEXED — run `sheepdog index`'}")
...
def _snapshot_worktree(workspace_root: str, snapshot_id: str) -> str:
    """Copy the current worktree into ``.sheepdog/snapshots/<snapshot_id>/``.
    ...
    """
    snap_dir = os.path.join(workspace_root, ".sheepdog", "snapshots", snapshot_id)
...
def _launch_agent(
    ...
    missing).
    """
    cfg = load_config(os.path.join(workspace_root, ".sheepdog", "config.yaml"))
    ...
    if agent_name == "opencode":
        xdg_base = os.path.join(workspace_root, ".sheepdog", "xdg")
    ...
    extra_env["SHEEPDOG_EXECUTION_ID"] = execution.execution_id
...
def cmd_exec_shim(args):
    """Internal: called by workspace shim scripts to launch an agent under Sheepdog.

    Usage: sheepdog _exec_shim <agent_name> <workspace_root> [argv...]
    """
...
def cmd_inspect(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg_path = os.path.join(ws_root, ".sheepdog", "config.yaml")
    ...
    if getattr(args, "json", False):
        ...
        print(f"Sheepdog Workspace: {project_name}\n")
        print("COMPARTMENTS\n")
...
def cmd_run(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg = load_config(os.path.join(ws_root, ".sheepdog", "config.yaml"))
    if cfg.workflows:
        print("Available Workflows:")
        for wname in sorted(cfg.workflows):
            print(f"  - {wname:<24} (run with: sheepdog run {wname})")
        return
    ...
    if os.path.exists(TOPOLOGY_FILE):
        ...
        if comps:
            ...
            print("Use `sheepdog workflow show` or `sheepdog check` for maintenance. `sheepdog run <workflow>` to execute.")
            print("No fallback execution performed.")
            return

    print("No workflow specified. Usage: sheepdog run <workflow_name>")


def cmd_exec(args):
    """Run a command or launch an agent inside an explicitly selected compartment."""
    if not args.cmd:
        print("Error: No command provided to exec. Usage: sheepdog exec [--compartment X] <command>")
        sys.exit(1)
    ...
    result = runner.run(
        ...
        env={"SHEEPDOG_EXECUTION_ID": execution.execution_id},
    )
    if result.stdout:
        print(result.stdout)
    if result.stderr:
...
def cmd_wrap(args):
    ...
    if args.claude is not None:
        ...
        xdg_base = os.path.join(ws_root, ".sheepdog", "xdg")
        for sub in ["config", "data", "cache", "state"]:
            os.makedirs(os.path.join(xdg_base, sub), exist_ok=True)
        env_vars.update({
            "XDG_CONFIG_HOME": os.path.join(xdg_base, "config"),
    ...
    env_vars["SHEEPDOG_EXECUTION_ID"] = execution.execution_id

    print(f"Sheepdog Agent Session #{session.session_id} active on Virtual Agent Lane '{lane_id}'.")
...
def cmd_lanes(args):
    ...
    lanes = lane_mgr.list_lanes()
    if not lanes:
        print("No virtual agent lanes found. Run 'sheepdog lane create <name>' or 'sheepdog wrap --lane <name>' first.")
        return
...
def cmd_lane_inspect(args):
    ...
    if args.json:
        ...
        print("================================================================")
        print(f"              SHEEPDOG VIRTUAL AGENT LANE #{lane.lane_id}        ")
        print("================================================================")
...
def cmd_sessions(args):
    ...
    if not sessions:
        print("No recorded agent sessions found. Run 'sheepdog wrap' or 'sheepdog run' first.")
        return
...
def _run_declared_workflow(ws_root: str, wf: WorkflowConfig, cfg: WorkspaceConfig) -> None:
    ...
    exec_mgr = ExecutionManager(workdir=ws_root)
    print(f"\nSHEEPDOG WORKFLOW: {wf.name}  ({len(nodes)} node(s))\n")
    for n in nodes:
    ...
    for n in nodes:
        ...
        result = runner.run(
            ...
            env={"SHEEPDOG_EXECUTION_ID": ex.execution_id},
        )
        if result.stdout:
...
def _run_workflow_file(ws_root: str, workflow_file: str, compartment_id: str, verbose: bool) -> None:
    ...
    print(f"Sheepdog Workflow Execution #{execution.execution_id}")
...
def cmd_workflow_branch(args):
    ...
    if not name:
        print("Error: Workflow branch name required. Usage: sheepdog workflow branch <name>")
        sys.exit(1)
    ...
    print(f"  To add steps:  sheepdog step {name} <file_or_command>")
    print(f"  To execute:    sheepdog workflow run {name}")
...
def cmd_diff(args):
    ...
    print("\nSHEEPDOG DIFF - changes associated with agent executions\n")
    if not records:
        print("  (no change sets recorded - run an agent under Sheepdog first)")
        print()
        return
...
def _git_commit_execution(
    ...
    msg_header = user_message or f"Sheepdog: apply changes from {ex.agent_name} ({ex.execution_id})"
...
def cmd_apply(args):
    ...
    if getattr(args, "commit", False):
        print("\nCreating Git commit(s) with Sheepdog metadata trailers...")
...
def cmd_undo(args):
    """Reverse the last Sheepdog-managed apply operation and restore the worktree."""
    ws_root = find_workspace_root()
    if not ws_root:
        print("Error: Not inside a Sheepdog workspace. Run 'sheepdog init' first.")
        sys.exit(1)
...
def cmd_inventory(args):
    ...
    if critical > 0:
        ...
        print("   Run `sheepdog autopatch` to generate verified migration PRs.")
...
def cmd_verify(args):
    ...
    print("================================================================================")
    print("                    SHEEPDOG AUTOPATCH: BEHAVIORAL VERIFIER                      ")
    print("================================================================================\n")
...
def _print_auth_warning():
    print("================================================================================")
    print("                SHEEPDOG: AI PROVIDER AUTHENTICATION REQUIRED                   ")
    print("================================================================================\n")
    print("[ERROR] No AI provider configured.\n")
    print("Sheepdog requires an AI provider to analyze dependencies and maintain your code.")
...
def cmd_reviews(args):
    """List past Sheepdog maintenance runs from the repository ledger."""
    root_path = os.path.abspath(getattr(args, "path", ".") or ".")
    history = get_migration_history(root_path)
    if not history:
        print("No Sheepdog maintenance runs recorded for this repository yet.")
        print("Run `sheepdog fix` to create the first entry.")
        return
    print(f"Sheepdog maintenance runs ({len(history)}):")
...
def cmd_auth(args):
    ...
    if getattr(args, "status", False):
        summary = get_active_provider_summary()
        print("================================================================================")
        print("                 SHEEPDOG: AI PROVIDER CREDENTIAL STATUS                         ")
        print("================================================================================\n")
        if summary["configured"]:
            ...
            print("Action:       Run 'sheepdog auth' to connect an AI provider.")
        print("\n================================================================================")
        return

    if getattr(args, "clear", False):
        clear_credentials()
        print("[OK] Stored Sheepdog credentials removed.")
        return
    ...
    if not provider or not api_key:
        print("================================================================================")
        print("                   SHEEPDOG: CONNECT YOUR AI PROVIDER                            ")
        print("================================================================================\n")
        ...
        if not sys.stdin.isatty() and (not provider or not api_key):
            ...
            print("  sheepdog auth --provider <anthropic|openai> --api-key <sk-...>")
            sys.exit(1)
    ...
    root_path = os.path.abspath(getattr(args, "path", ".") or ".")
    print(f"\n[INDEXING] Initializing Sheepdog Knowledge Graph for: {root_path}...")
    try:
        ...
        print("\nNext steps:")
        print("  1. Run `sheepdog check` to inspect external dependencies and drift.")
        print("  2. Run `sheepdog fix` to autonomously resolve migrations.")
...
def cmd_consult(args):
    ...
    if not has_valid_credentials():
        _print_auth_warning()
        print("Consult reasons with AI — static `sheepdog check` needs no key.")
        sys.exit(1)

    detections = [d for d in detect_changes(root_path) if d.outcome != NO_IMPACT]
    if not detections:
        print("Sheepdog Consult: no maintenance problems found. No changes made.")
        return
    ...
    title = f"[Sheepdog Consult] {len(items)} maintenance issue(s) in {repo}"
    resp = client.create_issue(repo=repo, title=title, body=body, labels=["sheepdog", "consult"])
...
def cmd_app(args):
    ...
    if action == "serve":
        ...
        secret = args.secret or os.environ.get("SHEEPDOG_WEBHOOK_SECRET")
        if not secret and not getattr(args, "no_secret", False):
            ...
            print("Set SHEEPDOG_WEBHOOK_SECRET, pass --secret, or use --no-secret for local debugging only.")
            sys.exit(2)
        ...
        print("================================================================================")
        print("               SHEEPDOG GITHUB APP: CONTINUOUS WEBHOOK LISTENER                  ")
        print("================================================================================\n")
        ...
        try:
            ...
            print("\n[Sheepdog App] Stopped.")
    elif action == "status":
        print("[Sheepdog App] Status: Ready (Daemon not running locally).")


def cmd_mcp(args):
    """Run Sheepdog Model Context Protocol (MCP) server over stdio."""
    serve_stdio()
...
def cmd_maintain(args):
    ...
    print("================================================================================")
    print("               SHEEPDOG AUTONOMOUS MAINTENANCE LOOP: EXECUTION                   ")
    print("================================================================================\n")
    ...
    if not target_provider or target_provider == "auto":
        ...
        if detected_all:
            ...
            print("Run `sheepdog check` to audit, or pass --provider explicitly.")
            print("================================================================================")
            return
    ...
    if getattr(report, "error", None):
        ...
        print("Action required:       Run `sheepdog auth` to enable AI repair.")
...
def cmd_graph(args):
    ...
    print("================================================================================")
    print("                 SHEEPDOG: EXTERNAL-CHANGE DEPENDENCY GRAPH                     ")
    print("================================================================================\n")
...
def cmd_providers(args):
    ...
    print("================================================================================")
    print("                    SHEEPDOG PROVIDER CONTRACT REGISTRY                          ")
    print("================================================================================\n")
    for p in providers:
...
def cmd_pr(args):
    """Review a pull request using the Sheepdog PR Bot."""
    ...
    print("================================================================================")
    print("                      SHEEPDOG AUTOMATED PR CONTRACT GUARD                       ")
    print("================================================================================\n")
...
def main():
    # Direct agent shortcut: `sheepdog claude` / `sheepdog opencode` / `sheepdog codex`
    ...
    description = textwrap.dedent("""\
        Sheepdog: Autonomous External-Change Intelligence & Controlled Execution
        ...
        Core Commands (maintenance product — keeps software working when systems around it change):
          ...
          sheepdog check [path]             Detect contract changes & impact (read-only, alias: scan, audit)
          ...
          sheepdog app serve                Run GitHub App webhook listener
          sheepdog pr                       Review a pull request with contract guard

        Legacy / advanced (workflows, sessions, lanes):
          sheepdog init | status | diff | apply | commit | undo | graph | exec | mcp
    """)
    ...
    parser.add_argument(
        ...
        dest="workflow_flag",
        default=None,
        help="Create a new workflow branch (e.g. sheepdog -w invoice-pipeline)",
    )
    parser.add_argument(
        "--run",
        dest="run_flag",
        default=None,
        help="Run a workflow DAG directly (e.g. sheepdog --run invoice-pipeline)",
    )
    subparsers = parser.add_subparsers(dest="command", help=argparse.SUPPRESS)

    subparsers.add_parser("init", help="Initialize a Sheepdog workspace in the current directory")
    ...
    apply_parser.add_argument("-c", "--commit", action="store_true", help="Automatically create a Git commit with Sheepdog trailers")
    ...
    undo_parser = subparsers.add_parser("undo", help="Reverse the last Sheepdog-managed apply operation")
    ...
    explain_p = subparsers.add_parser("explain", help="Explain why Sheepdog classified a finding as affected, unaffected, or unresolved")
    ...
    check_p.add_argument("--write-graph", action="store_true", help="Persist .sheepdog/graph.json")
    ...
    scan_p.add_argument("--write-graph", action="store_true", help="Persist .sheepdog/graph.json")
    ...
    audit_p.add_argument("--write-graph", action="store_true", help="Persist .sheepdog/graph.json")
    ...
    reviews_p = subparsers.add_parser("reviews", help="List past Sheepdog maintenance runs")
    ...
    app_p = subparsers.add_parser("app", help="Manage Sheepdog GitHub App and Webhook server")
    ...
    subparsers.add_parser("mcp", help="Run Sheepdog Model Context Protocol (MCP) server for AI assistants")
    ...
    index_p.add_argument("--write-graph", action="store_true", default=True, help="Persist .sheepdog/graph.json")
