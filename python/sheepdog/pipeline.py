...
"""
Sheepdog event model and shared maintenance pipeline.
...
from sheepdog.github.client import GitHubAppClient
from sheepdog.credentials import has_valid_credentials
...
from sheepdog.autopatch import (
    ScanConfig,
    scan_callsites,
)
from sheepdog.providers.registry import get_default_registry, ProviderSpec
from sheepdog.drift import detect_drift
from sheepdog.formatters import run_style_formatter
from sheepdog.test_runner import (
    _detect_test_command,
    _run_tests,
    _compute_lockfile_hash,
    _blake3_digest,
)
from sheepdog.intelligence import SheepdogIntelligence, resolve_migration
from sheepdog.knowledge import direct_rewrites_for, upsert_learned as kb_upsert
...
from sheepdog.patch_writer import apply_rewrites, discover_aliases, instantiate_alias_rules
from sheepdog.sandbox.snapshot import SnapshotManager

_logger = logging.getLogger("sheepdog.pipeline")
    ...
    def run(self, ctx: TriggerContext) -> "PipelineResult":
        ...
        if not analysis.has_findings and not self.policy.always_report_clean:
            return PipelineResult(
                ...
                status="clean",
                comment_body="",
                status_description="Sheepdog: no external contract impact detected",
            )
    ...
    def _run_consult(self, ctx: TriggerContext, analysis: AnalysisResult) -> "PipelineResult":
        ...
        if not has_valid_credentials():
            return PipelineResult(
                ...
                analysis=analysis,
                status="refused",
                comment_body="Sheepdog Consult requires AI reasoning — no provider configured. "
                             "Run `sheepdog auth` to connect one.",
                status_description="Sheepdog Consult refused: no AI provider configured",
            )
        ...
        return PipelineResult(
            ...
            status="consulted",
            comment_body=body,
            status_description=f"Sheepdog Consult: assessed {len(items)} maintenance issue(s), no code modified",
        )
...
def apply_fixes(
    ...
    """Apply fixes via Sheepdog Intelligence: DIRECT (registry+KB) or AI fallback. Single router."""
    modified_files: List[str] = []
    unified_diffs: List[str] = []

    snapshot_dir = os.path.join(ctx.workdir, ".sheepdog", "snapshot_tmp")
...
def surface_result(
    ...
    if analysis.verified and analysis.modified_files:
        ...
        if analysis.test_command and analysis.test_exit_code == 0:
            status_desc = f"Sheepdog: autonomous repair verified ({len(analysis.modified_files)} file(s) patched, tests green)"
        else:
            status_desc = f"Sheepdog: autonomous repair applied ({len(analysis.modified_files)} file(s) patched, test suite unconfigured)"
        ...
        status_desc = f"Sheepdog found {len(analysis.findings)} maintenance issue(s)"
    else:
        comment = render_verification_comment(analysis, ctx)
        status_desc = "Sheepdog: no external contract impact detected"
