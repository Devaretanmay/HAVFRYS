"""Sheepdog - kernel-level sandbox with an insulated runtime for isolated execution.

Two entry points:

* :class:`Sheepdog` : a normal outer compartment. A kernel sandbox plus a runtime that
...
* :class:`AgentSheepdog` : an agent outer compartment (agent-oriented container). It auto-loads every
...
_logger = logging.getLogger("sheepdog")


@dataclass
class SheepdogConfig:
    workdir: str = "."
...
class Sheepdog:
    ...
    SHEEPDOG_DIR = ".sheepdog"

    def __init__(
        self,
        workdir: str = ".",
        config: Optional[SheepdogConfig] = None,
        verbose: bool = False,
    ):
        if config is not None:
            self.config = config
        else:
            self.config = SheepdogConfig(workdir=workdir)
        self.workdir = os.path.abspath(self.config.workdir)
        self._snapshot_base = os.path.join(self.workdir, self.SHEEPDOG_DIR, "snapshots")
    ...
    def add(self, compartment: Compartment) -> "Sheepdog":
    ...
    def edge(self, from_name: str, to_name: str) -> "Sheepdog":
    ...
    def register_module(self, module_cls) -> "Sheepdog":
    ...
    def run(
        ...
        SheepdogResult
        """
        task_desc = request or entry or "sheepdog.run()"
        self._started_at = time.time()
        _logger.info("Sheepdog %s running: %s", self.box_id, task_desc[:80])
    ...
    def _build_result(self, raw: dict[str, Any], elapsed: float) -> SheepdogResult:
        errors: list[str] = []
        completed: list[str] = []
        status = "success"
...
class AgentSheepdog(Sheepdog):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            config = SheepdogConfig(workdir=workdir, auto_modules=True)
