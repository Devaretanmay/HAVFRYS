...
def generate_trust_pr_markdown(meta: TrustPRMetadata) -> str:
    ...
    summary_sentence = (
        ...
        f"Sheepdog found {found_count} affected callsite(s) -> patched {patched_count} -> "
        f"{test_summary} -> 0 unrelated files -> {status_tag}**"
    ) if found_count != patched_count else (
        f"**{meta.provider_name} {meta.from_version} -> {meta.to_version} {change_clause} -> "
        f"Sheepdog found {patched_count} affected callsite(s) -> patched those {patched_count} -> "
        f"{test_summary} -> 0 unrelated files -> {status_tag}**"
    )
    ...
    lines.extend([
        ...
        "Sheepdog verified zero unintended side effects across the entire codebase:",
    ...
    lines.extend([
        "",
        "_Generated automatically by [Sheepdog](https://github.com/Devaretanmay/Sheepdog) Continuous Autonomous Maintenance Engine._",
        "— Shearer, Work bot",
    ])
    return "\n".join(lines)
