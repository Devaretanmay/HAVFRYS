...
"""
Sheepdog GitHub App PR bot.

This is the first product milestone: install the app, open a PR, and Sheepdog
...
from sheepdog.ai_planner import AIPatchPlanner, build_reasoning_context
...
from sheepdog.github.installations import (
    REPO_INDEXED, REPO_PENDING, REPO_READY, record_installation_event, set_repo_state,
)
from sheepdog.github.provisioning import resolve_pr_workdir
from sheepdog.graph import build_dependency_graph
from sheepdog.drift import detect_drift

_logger = logging.getLogger("sheepdog.pr_bot")
...
def run_on_pr_locally(
    ...
    Useful for:
    * local development
    * CI check jobs that want Sheepdog to post results
    """
...
def render_day0_onboarding_issue(repo: str, workdir: Optional[str] = None) -> str:
    ...
    lines = [
        "-----------------------------------------",
        "   SHEEPDOG DAY-0 REPOSITORY ONBOARDING   ",
        "-----------------------------------------",
        "",
        f"Sheepdog has mapped external API dependencies and contracts for `{repo}`.",
        "",
        ...
        "  - [OK] Auto-Fix Policy: Opt-in (Configurable via .sheepdog/config.yaml)",
...
def handle_installation_event(
    ...
    for repo_info in repos_data:
        ...
        try:
            client.create_issue(
                repo=repo_name,
                title="[SHEEPDOG] Day-0 External Contract & API Dependency Map",
                body=issue_body,
                labels=["sheepdog", "maintenance"],
            )
...
def handle_issue_comment_event(
    ...
    """Handle issue_comment.* events mentioning @sheepdog.

    `@sheepdog` alone re-runs the pipeline on the PR; `@sheepdog explain`
    ...
    if not repo or not number or not body or "@sheepdog" not in body.lower():
    ...
    if "@sheepdog explain" in body.lower():
        ...
        reply = render_consult_issue(items) if items else (
            "Sheepdog checked this PR's external API touchpoints. "
