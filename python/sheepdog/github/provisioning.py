...
otherwise (public repos), or SHEEPDOG_REPO_REMOTE_* overrides (tests,
mirrors, self-hosted).
"""
...
_logger = logging.getLogger("sheepdog.provisioning")


def cache_dir() -> str:
    base = os.environ.get("SHEEPDOG_REPOS_DIR",
                           os.path.join(os.path.expanduser("~/.sheepdog"), "repos"))
...
def remote_for(repo_full_name: str, token: Optional[str] = None) -> str:
    ...
    override = os.environ.get(f"SHEEPDOG_REPO_REMOTE_{slug}")
    if override:
        return override
    if token:
