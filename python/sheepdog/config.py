"""Sheepdog workspace configuration loader.

Reads `.sheepdog/config.yaml` and returns typed config objects for compartments,
...
@dataclass
class BotConfig:
    """Sheepdog GitHub App behavior policy.
...
def load_config(config_path: Optional[str] = None) -> WorkspaceConfig:
    """Load `.sheepdog/config.yaml`.  Returns safe defaults when not found."""
    if config_path is None:
        config_path = os.path.join(".sheepdog", "config.yaml")
    ...
    if not _YAML_AVAILABLE:
        warnings.warn(
            ...
            "to use .sheepdog/config.yaml. Falling back to default policy.",
            RuntimeWarning,
            stacklevel=2,
        )
        return _default_config()
    ...
    for dir_name in ("workflows", os.path.join(".sheepdog", "workflows")):
...
def is_sheepdog_workspace(path: Optional[str] = None) -> bool:
    """Return True if *path* (or cwd) is inside a Sheepdog workspace."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".sheepdog")):
            return True
...
def find_workspace_root(path: Optional[str] = None) -> Optional[str]:
    """Walk up the directory tree looking for a .sheepdog/ directory."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".sheepdog")):
            return check
