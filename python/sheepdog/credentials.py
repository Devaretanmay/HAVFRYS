...
"""Secure BYOK AI provider credential management for Sheepdog."""
...
CREDENTIALS_DIR = os.path.expanduser("~/.sheepdog")
CREDENTIALS_FILE = os.path.join(CREDENTIALS_DIR, "credentials.json")


def get_credentials_path() -> str:
    """Return path to user's credentials file, allowing override via env."""
    return os.environ.get("SHEEPDOG_CREDENTIALS_FILE", CREDENTIALS_FILE)
...
def has_valid_credentials(installation_id: Optional[str] = None, repo: Optional[str] = None) -> bool:
    ...
    if os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("OPENAI_API_KEY") or os.environ.get("SHEEPDOG_LLM_KEY"):
        return True
...
def get_active_provider_summary() -> Dict[str, Any]:
    ...
    if os.environ.get("SHEEPDOG_LLM_KEY"):
        return {"configured": True, "provider": "custom", "source": "env:SHEEPDOG_LLM_KEY"}
