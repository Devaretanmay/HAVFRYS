"""Sheepdog CI Step Runner & Wrapper.

Allows 1-word or 1-line execution of CI pipeline steps inside a kernel-enforced
Sheepdog compartment. Preserves stdout/stderr and exit codes for CI runners.
"""
...
from sheepdog.sheepdog import AgentSheepdog, SheepdogConfig
from sheepdog.compartments import Compartment, CompartmentConfig


class SheepdogCIRunner:
    ...
    def run_step(self, cmd: str, name: str = "ci_step") -> dict[str, Any]:
        """Runs a command inside a kernel-enforced Sheepdog CI compartment."""
...
def run_ci_step(cmd: str, block_network: bool = True, sandbox: bool = True, timeout_s: int = 600) -> int:
    ...
    runner = SheepdogCIRunner(block_network=block_network, sandbox=sandbox, timeout_s=timeout_s)
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="sheepdog",
        description="Sheepdog CI Runner - 1-word drop-in kernel sandbox for CI pipeline steps.",
    )
