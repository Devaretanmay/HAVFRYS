...
* :class:`SheepdogPythonREPLTool` : a drop-in replacement for ``PythonREPLTool``
  that executes the code snippet inside a kernel-enforced Sheepdog
  compartment instead of ``exec()`` in the caller's process.
* :class:`SheepdogGraphNode` : wraps any LangGraph execution node so its body
  runs inside an :class:`sheepdog.AgentSheepdog` compartment, with
...
class SheepdogPythonREPLTool(BaseTool):  # type: ignore[misc]
    ...
    description: str = (
        ...
        "Runs inside a kernel-enforced Sheepdog sandbox."
    )
...
class SheepdogGraphNode:
    """Wrap a function so it runs inside a Sheepdog compartment.

    Turns any callable into a LangGraph node that executes within a
    kernel-enforced :class:`sheepdog.AgentSheepdog`. The wrapped node calls
    ...
    def _permissions_from(self, metadata: Optional[Mapping[str, Any]]) -> tuple[str, ...]:
        ...
        if metadata:
            listed = metadata.get("permissions") or metadata.get("sheepdog_permissions")
            if listed:
    ...
    def attach(
        ...
        *,
        ...
        if builder is None:
            raise TypeError(
                "SheepdogGraphNode.attach requires langgraph; install it with"
                " 'pip install langgraph'."
            )
        node_name = name or getattr(self._fn, "__name__", "sheepdog_node")
