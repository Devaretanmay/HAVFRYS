...
```mermaid
flowchart LR
    A["Install GitHub App"] --> B["Connect AI Provider"]
    B --> C["Zero-Token Index"]
    C --> D["Webhook PR Event / @howl / @hunt"]
    D --> E["Howl: Advisory Review<br/>(Reads, warns, files issues)"]
    D --> F["Hunt: Autonomous Repair<br/>(AI patches, tests in sandbox, opens PR)"]
```
...
```mermaid
flowchart TD
    EVENT["WEBHOOK PR / CLI TRIGGER<br/>pull_request.opened / @howl / @hunt"] --> DETECT["DETECT IMPACT<br/>AST graph · callsites · zero-token"]
    DETECT --> MEM["SEMANTIC MEMORY<br/>verified patterns in .koyote/knowledge/<br/>saves customer tokens"]
    MEM --> AI["AI GENERATION & REASONING<br/>customer BYOK provider<br/>sole author of changes & reviews"]
    AI -->|Howl Advisor| ADVICE["HOWL: ADVISORY REVIEW<br/>explains drift · files Issue / PR comment · 0 files touched"]
    AI -->|Hunt Worker| PATCH["HUNT: SURGICAL PATCH<br/>generated by user's AI provider"]
    PATCH --> TEST["NATIVE VERIFICATION<br/>kernel sandbox · real tests · zero blast radius"]
