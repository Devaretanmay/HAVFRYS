# Deploying the Compart GitHub App Daemon
...
|---|---|---|
| `COMPART_WEBHOOK_SECRET` | **yes** | HMAC validation. The daemon refuses to serve without it (`--no-secret` is local-debug only). |
| `GITHUB_TOKEN` or App `COMPART_GITHUB_APP_ID` + `COMPART_GITHUB_PRIVATE_KEY` | yes for private repos / PR writes | Clone auth and PR comments. Public repos work anonymously for clones. |
...
| `COMPART_REPOS_DIR`, `COMPART_INSTALLATIONS_DIR` | no | Managed checkouts and install records. Persist both (volume `/data`). |

## 3. Run with Docker

```bash
docker build -f docker/Dockerfile.app -t compart-app:1.1.0 .
docker run -d --name compart -p 8080:8080 --env-file .env -v compart-data:/data compart-app:1.1.0
```

With background monitoring (poll READY repos every 5 minutes):

```bash
docker run -d --name compart -p 8080:8080 --env-file .env -v compart-data:/data \
  compart-app:1.1.0 sh -c "compart app serve --port ${PORT:-8080} --watch 300"
```
...
```bash
pip install compart
export COMPART_WEBHOOK_SECRET=... GITHUB_TOKEN=...
compart app serve --port 8080 --watch 300
```

## 5. Verify

- `GET /health` → `{"status": "healthy"}`.
- `compart doctor` on the host shows GitHub CONNECTED once token env is set.
