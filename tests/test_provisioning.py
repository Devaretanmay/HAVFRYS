...
from compart.github.provisioning import (
...
def test_clone_and_cached_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("COMPART_REPO_REMOTE_ACME__BACKEND", _seed_remote(tmp_path))
...
def test_unreachable_remote_returns_none(tmp_path, monkeypatch):
    monkeypatch.setenv("COMPART_REPOS_DIR", str(tmp_path / "repos"))
    monkeypatch.setenv("COMPART_REPO_REMOTE_GHOST__NOPE", "/nonexistent/path/xyz")
...
def test_remote_for_prefers_token_and_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("COMPART_REPO_REMOTE_A__B", "/mirror/b")
