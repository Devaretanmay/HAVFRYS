...
def test_issue_comment_bot_sender_ignored():
    ...
    res = handle_issue_comment_event(
        _comment_payload("@compart please", sender_type="Bot"), "issue_comment.created", client)
    assert res["handled"] is False
...
def test_issue_comment_rerun_delegates_to_pr_handler():
    ...
    with patch("compart.github.pr_bot.handle_pull_request_event") as mock_pr:
        mock_pr.return_value = {"success": True}
        res = handle_issue_comment_event(
            _comment_payload("@compart please re-run"), "issue_comment.created", client)
...
def test_issue_comment_explain_without_creds_refuses(tmp_path, monkeypatch):
    ...
    res = handle_issue_comment_event(
        _comment_payload("@compart explain this"), "issue_comment.created", client,
...
def test_issue_comment_explain_posts_assessment(tmp_path, monkeypatch):
    ...
    import compart.github.pr_bot as pr_bot_mod
