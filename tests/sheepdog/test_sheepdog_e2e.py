"""E2E tests for the compartment-centric Sheepdog API."""
...
from sheepdog.sheepdog import Sheepdog, AgentSheepdog
from sheepdog.compartments import Compartment, CompartmentConfig
from sheepdog.sandbox.credential_module import CredentialModule
...
class TestSheepdogInit(unittest.TestCase):

    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"sheepdog_e2e_{uuid.uuid4().hex[:8]}")
    ...
    def test_plain_box_loads_no_modules_by_default(self):
        """Sheepdog is just the box: no modules auto-load."""
        box = AgentSheepdog(workdir=self.tmpdir)
        ...
        self.assertGreater(module_count, 0, "AgentSheepdog should auto-load modules")

        plain = Sheepdog(workdir=self.tmpdir)
        plain._box.insulate("Fix the bug")
        plain_count = sum(len(e.modules) for e in plain._box._engines.values())
        self.assertEqual(plain_count, 0, "plain Sheepdog ships empty")
...
class TestCompartmentFlow(unittest.TestCase):
    ...
    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"sheepdog_flow_{uuid.uuid4().hex[:8]}")
        _make_repo(self.tmpdir)
