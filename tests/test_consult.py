...
from compart.config import BotConfig, PipelinePolicy, load_config
...
def test_assess_shares_context_with_repair(tmp_path):
    from compart.ai_planner import build_reasoning_context
    dst = str(tmp_path / "r")
    _seed_repo(dst)
...
def test_consult_refuses_without_credentials(tmp_path):
    res = subprocess.run(
        [sys.executable, "-m", "compart.cli.main", "consult", "."],
...
def test_consult_opens_issue_without_modifying(tmp_path, monkeypatch):
    from compart.cli import main as cli_main
    dst = str(tmp_path / "r")
    _seed_repo(dst)
    monkeypatch.setenv("COMPART_CREDENTIALS_FILE", str(tmp_path / "creds.json"))
...
def test_pipeline_consult_branch_reports_without_patching(tmp_path, monkeypatch):
    from compart.pipeline import MaintenancePipeline, TriggerContext
    dst = str(tmp_path / "r")
    _seed_repo(dst)
    monkeypatch.setenv("COMPART_CREDENTIALS_FILE", str(tmp_path / "creds.json"))
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "COMPART_LLM_KEY"):
...
def test_pipeline_consult_assesses_with_mock_planner(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("COMPART_CREDENTIALS_FILE", str(tmp_path / "creds.json"))
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-testkey1234567890")
    import compart.pipeline as pipe_mod
